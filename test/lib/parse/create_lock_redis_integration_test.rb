...
class CreateLockRedisIntegrationTest < Minitest::Test
  ...
  def test_clear_rejects_non_string_scope
    # Symbols / integers / nil-via-to_s would silently expand the
    # deletion target (`scope: 0` -> "0:*"). Must raise instead.
    assert_raises(ArgumentError) { @wrapper.clear(scope: :tenant_x) }
    assert_raises(ArgumentError) { @wrapper.clear(scope: 0) }
    assert_raises(ArgumentError) { @wrapper.clear(scope: ["tenant_x"]) }
  end

  def test_clear_rejects_glob_metacharacters_in_scope
    # The critical safety property: a glob in scope: must NOT pass through
    # to redis.scan(match:) and trigger a whole-DB SCAN-DEL. flush_db! is
    # the only documented path for that blast radius.
    @probe.store("anything:1", "x", expires: 60)
    @probe.store("parse-stack:foc:v1:fixture", "owner", expires: 60)

    %w[* ? *bar foo* foo[bar] foo\\bar].each do |bad|
      assert_raises(ArgumentError, "scope: #{bad.inspect} must be rejected") do
        @wrapper.clear(scope: bad)
      end
    end

    # Confirm none of the rejected calls leaked deletion to Redis.
    assert_equal "x", @probe["anything:1"]
    assert_equal "owner", @probe["parse-stack:foc:v1:fixture"]
  end
