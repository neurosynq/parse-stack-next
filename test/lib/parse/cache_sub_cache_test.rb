...
class CacheSubCacheTest < Minitest::Test
  ...
  # own; the plane has to apply one itself or the counter is immortal on
  # exactly the deployments that matter.
  def test_atomic_increment_path_still_applies_the_expiry
    ...
    key = store.options.keys.find { |k| k.include?(":gen:") }
    refute_nil key, "the atomic path must still have written an expiry"
    assert_equal 7200, store.options[key][:expires]
  end
end
